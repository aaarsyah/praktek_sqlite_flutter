...
import 'package:prakteksqlite/pages/pelanggan_form.dart';
import 'package:prakteksqlite/pages/pelanggan_cari.dart';
...
class _PelangganListState extends State<PelangganList> {
  ...
  Widget item(Map d) => ListTile(
        onLongPress: () {
          showMenu(
              ...
              items: [
                ...
                PopupMenuItem(child: Text('Hapus data ini'), value: 'H')
              ]).then((value) {
            if (value == 'S') {
        ...
        leading: PopupMenuButton(
            itemBuilder: (bc) => [
                  PopupMenuItem(child: Text('Sunting data ini'), value: 'S'),
                  PopupMenuItem(child: Text('Hapus data ini'), value: 'H'),
                ],
            onSelected: (value) {
              if (value == 'S') {
                Navigator.push(
                    context,
                    MaterialPageRoute(
                        builder: (c) => PelangganForm(data: d))).then((value) {
                  if (value = true) refresh();
                });
              } else if (value == 'H') {
                showDialog(
                    ...
                    builder: (c) => AlertDialog(
                          content:
                              Text('Pelanggan ${d['nama']} ingin dihapus?'),
                          actions: [
                            TextButton(
                                onPressed: () {
                                  hapusData(d['id']).then((value) {
